...
Add an approval policy to a tool and register the same built agent in Studio:
...
Studio handles the approval UI for tools with an `approval` policy. Your TanStack Start app can still expose its own routes for production users.
...
Add route tests in [Setup Tests](/docs/frameworks/tanstack-start/10-setup-tests). Core concepts: [Human in the Loop](/docs/guides/human-in-the-loop), [Hook Approval Requests](/docs/guides/human-in-the-loop/tool-approval), [Approval Handlers](/docs/guides/human-in-the-loop/approval-handlers), and [Studio Tool Approvals](/docs/studio/human-in-the-loop/tool-approvals).
